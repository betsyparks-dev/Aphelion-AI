...
/** Count businesses generated by a user this month (gracefully handles missing DB) */
export async function getUserMonthlyBusinessCount(userId: string): Promise<number> {
  const { sql } = await import("~/db");
  try {
    const db = sql();
    ...
    // If DB is unavailable, allow the generation (graceful fallback)
    return 0;
  }
}

/** Get the generation limit for a subscription tier */
...
/** Check if a user can generate a business and return a limit error message if not */
export async function checkGenerationLimit(userId: string, tier: string): Promise<string | null> {
  const limit = getGenerationLimit(tier);
  if (limit === Infinity) return null; // Pro users: unlimited

  const count = await getUserMonthlyBusinessCount(userId);
  if (count >= limit) {
    const planName = tier === "free" ? "Free" : "Starter";
    const upgradePlan = tier === "free" ? "Starter" : "Pro";
    return `You've reached the ${planName} plan limit of ${limit} business${limit === 1 ? "" : "es"} per month. Upgrade to ${upgradePlan} to generate more.`;
  }
